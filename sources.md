...
1. **English only.** The piece must be published in English by the outlet itself.
   Never translate. A non-English publication with a genuine English edition
   qualifies; an English homepage wrapping local-language articles does not.
2. **Everything below was confirmed active** by fetching the site and seeing a
   2026-dated piece. Entries marked `UNVERIFIED` blocked automated fetching —
   do not cite them without checking manually.
...
Feeds marked **FILTER** mix criticism with commercial content — never take a
piece from them without checking it is actually criticism.
...
than dedicated verticals. East Asia has strong tailoring *culture* but almost no
local English-language *criticism*. Expect to lean on filtered general feeds.
...
3. **Never source from generic web search alone.** The first hand-made draft did,
   and produced two duplicated articles plus one fabricated summary.
